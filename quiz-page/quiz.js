let selectedCategory = localStorage.getItem("preferences");
let selectedDificulty = localStorage.getItem("difficult");
const body = document.querySelector("body");
const main = document.querySelector("main");
const categoryTitle = document.querySelector(".category-tittle");
const levelDifficulty = document.querySelector(".level-dificulty");
const contentBox = document.querySelector(".content-box");

setColorTheme(selectedCategory);
setDifficulty(selectedDificulty);

let answers = document.querySelectorAll(".answer");
let skipButton = document.getElementById("next-question");
let explanationButton = document.querySelector(".explanation");
let progress = document.getElementById("progress");
let progressBar = document.getElementById("progress-bar");
let h2 = document.querySelector("h2");
let timer = document.getElementById("timer");
let explainMenu = document.querySelector(".explain-menu");
let quitExplainButton = document.getElementById("quit-explain");
let nextExplainButton = document.getElementById("next-question");

let seconds = 15;
let stopTimer = false;
let answered = false;
let userSelection = [];
let currentExplanation = "";
let correctAnswer = "";
let skips = 0;
let correct = 0;
let id = 0;

let globalCategories = {
  history: [
    [
      {
        "Who was the first president of the United States?": {
          "George Washington": true,
          "John Adams": false,
          "Thomas Jefferson": false,
          "Benjamin Franklin": false,
        },
        explanation:
          "George Washington was the first president, serving from 1789 to 1797.",
      },
      {
        "In what year did the French Revolution begin?": {
          1789: true,
          1776: false,
          1804: false,
          1812: false,
        },
        explanation:
          "The French Revolution began in 1789, marking a period of radical social and political change.",
      },
      {
        "Which empire built the Colosseum in Rome?": {
          "The Roman Empire": true,
          "The Byzantine Empire": false,
          "The Ottoman Empire": false,
          "The Persian Empire": false,
        },
        explanation: "The Roman Empire built the Colosseum between AD 70-80.",
      },
      {
        "What historical event is commemorated on the 4th of July in the United States?":
          {
            "The Independence of the United States": true,
            "The Civil War": false,
            "The arrival of the first settlers in America": false,
            "The signing of the Treaty of Versailles": false,
          },
        explanation:
          "The 4th of July marks the adoption of the Declaration of Independence in 1776.",
      },
      {
        "What was the primary purpose of the Council of Trent (1545-1563)?": {
          "To reform the Catholic Church and respond to the Protestant Reformation": true,
          "To expand the Ottoman Empire": false,
          "To establish peace among European states": false,
          "To gather leaders of the Protestant Reformation": false,
        },
        explanation:
          "The Council of Trent was a major part of the Counter-Reformation.",
      },
      {
        "Which treaty ended the Thirty Years' War in 1648?": {
          "Treaty of Westphalia": true,
          "Treaty of Utrecht": false,
          "Treaty of Versailles": false,
          "Treaty of Peace of Oliva": false,
        },
        explanation:
          "The Treaty of Westphalia established the principles of sovereign states.",
      },
      {
        "What caused World War I?": {
          "Assassination of Franz Ferdinand": true,
          "Rise of Nazis": false,
          "Russian Revolution": false,
          "Treaty of Versailles": false,
        },
        explanation:
          "The assassination of Archduke Franz Ferdinand in 1914 triggered World War I.",
      },
      {
        "Who built the pyramids of Giza?": {
          "Ancient Egyptians": true,
          Mayans: false,
          Aztecs: false,
          Romans: false,
        },
        explanation:
          "The Ancient Egyptians built the pyramids as tombs for pharaohs.",
      },
      {
        "First woman to win a Nobel Prize?": {
          "Marie Curie": true,
          "Rosa Parks": false,
          "Ada Lovelace": false,
          "Florence Nightingale": false,
        },
        explanation:
          "Marie Curie won the Nobel Prize in Physics in 1903 for her work on radioactivity.",
      },
      {
        "When did the Berlin Wall fall?": {
          1989: true,
          1991: false,
          1975: false,
          1961: false,
        },
        explanation:
          "The Berlin Wall fell in 1989, leading to the reunification of Germany.",
      },
    ],
  ],
  science: [
    [
      {
        "What is the chemical symbol for water?": {
          H2O: true,
          O2: false,
          CO2: false,
          HO: false,
        },
        explanation:
          "H2O is the chemical formula for water, representing two hydrogen atoms and one oxygen atom.",
      },
      {
        "What planet is known as the Red Planet?": {
          Mars: true,
          Jupiter: false,
          Venus: false,
          Saturn: false,
        },
        explanation:
          "Mars is called the Red Planet due to its reddish appearance caused by iron oxide on its surface.",
      },
      {
        "What is the speed of light?": {
          "300,000 km/s": true,
          "150,000 km/s": false,
          "100,000 km/s": false,
          "50,000 km/s": false,
        },
        explanation:
          "The speed of light in a vacuum is approximately 300,000 kilometers per second.",
      },
      {
        "What gas do plants absorb from the atmosphere?": {
          "Carbon dioxide": true,
          Oxygen: false,
          Nitrogen: false,
          Methane: false,
        },
        explanation:
          "Plants absorb carbon dioxide during photosynthesis to produce oxygen and glucose.",
      },
      {
        "Which planet is the largest in the solar system?": {
          Jupiter: true,
          Saturn: false,
          Earth: false,
          Neptune: false,
        },
        explanation:
          "Jupiter is the largest planet in the solar system, with a diameter of about 143,000 kilometers.",
      },
      {
        "What is the basic unit of life?": {
          Cell: true,
          Atom: false,
          Molecule: false,
          Organ: false,
        },
        explanation:
          "The cell is the basic unit of life, making up all living organisms.",
      },
      {
        "What gas do humans need to breathe?": {
          Oxygen: true,
          "Carbon dioxide": false,
          Nitrogen: false,
          Helium: false,
        },
        explanation:
          "Humans need oxygen to survive, which is used in the process of respiration.",
      },
      {
        "What force keeps us on the ground?": {
          Gravity: true,
          Magnetism: false,
          Friction: false,
          Electricity: false,
        },
        explanation:
          "Gravity is the force that attracts objects towards the center of the Earth, keeping us grounded.",
      },
      {
        "What is the center of an atom called?": {
          Nucleus: true,
          Proton: false,
          Electron: false,
          Neutron: false,
        },
        explanation:
          "The nucleus is the central part of an atom, containing protons and neutrons.",
      },
      {
        "What is the most abundant gas in Earth's atmosphere?": {
          Nitrogen: true,
          Oxygen: false,
          "Carbon dioxide": false,
          Argon: false,
        },
        explanation:
          "Nitrogen makes up about 78% of Earth's atmosphere, making it the most abundant gas.",
      },
    ],
  ],
  geography: [
    [
      {
        "What is the largest continent?": {
          Asia: true,
          Africa: false,
          Europe: false,
          "North America": false,
        },
        explanation:
          "Asia is the largest continent, covering about 30% of the Earth's land area.",
      },
      {
        "Which river is the longest in the world?": {
          Nile: true,
          Amazon: false,
          Yangtze: false,
          Mississippi: false,
        },
        explanation:
          "The Nile River in Africa is the longest river in the world, stretching over 6,650 kilometers.",
      },
      {
        "What is the capital of France?": {
          Paris: true,
          Berlin: false,
          Rome: false,
          Madrid: false,
        },
        explanation:
          "Paris is the capital and largest city of France, known for landmarks like the Eiffel Tower.",
      },
      {
        "Which country has the largest population?": {
          China: true,
          India: false,
          "United States": false,
          Indonesia: false,
        },
        explanation:
          "China has the largest population in the world, with over 1.4 billion people.",
      },
      {
        "What ocean is the deepest?": {
          "Pacific Ocean": true,
          "Atlantic Ocean": false,
          "Indian Ocean": false,
          "Southern Ocean": false,
        },
        explanation:
          "The Pacific Ocean is the deepest ocean, with the Mariana Trench reaching a depth of about 11,000 meters.",
      },
      {
        "Which desert is the largest in the world?": {
          Sahara: true,
          Gobi: false,
          Kalahari: false,
          Arabian: false,
        },
        explanation:
          "The Sahara Desert in Africa is the largest hot desert, covering 9.2 million square kilometers.",
      },
      {
        "What is the smallest country by land area?": {
          "Vatican City": true,
          Monaco: false,
          "San Marino": false,
          Liechtenstein: false,
        },
        explanation:
          "Vatican City is the smallest country in the world, with an area of just 44 hectares.",
      },
      {
        "Which mountain is the highest in the world?": {
          "Mount Everest": true,
          K2: false,
          Kangchenjunga: false,
          Lhotse: false,
        },
        explanation:
          "Mount Everest, located in the Himalayas, is the highest mountain, with a peak at 8,848 meters above sea level.",
      },
      {
        "Which US state has the most coastline?": {
          Alaska: true,
          Florida: false,
          California: false,
          Hawaii: false,
        },
        explanation:
          "Alaska has the most coastline of any US state, with over 10,000 kilometers of shoreline.",
      },
      {
        "What is the largest country by land area?": {
          Russia: true,
          Canada: false,
          China: false,
          "United States": false,
        },
        explanation:
          "Russia is the largest country in the world by land area, spanning over 17 million square kilometers.",
      },
    ],
  ],
  entretainment: [
    [
      {
        "Which movie won the Oscar for Best Picture in 2020?": {
          Parasite: true,
          1917: false,
          Joker: false,
          "Once Upon a Time in Hollywood": false,
        },
        explanation:
          "Parasite, a South Korean film, won the Oscar for Best Picture in 2020.",
      },
      {
        "Who is the creator of the Star Wars franchise?": {
          "George Lucas": true,
          "Steven Spielberg": false,
          "James Cameron": false,
          "J.J. Abrams": false,
        },
        explanation:
          "George Lucas is the creator of the Star Wars franchise, which began in 1977.",
      },
      {
        "Which TV show features a character named Walter White?": {
          "Breaking Bad": true,
          "The Sopranos": false,
          "Mad Men": false,
          Dexter: false,
        },
        explanation:
          "Breaking Bad is a TV show that follows the story of Walter White, a chemistry teacher turned drug dealer.",
      },
      {
        "What is the highest-grossing film of all time?": {
          Avatar: true,
          "Avengers: Endgame": false,
          Titanic: false,
          "The Lion King": false,
        },
        explanation:
          "Avatar, directed by James Cameron, is the highest-grossing film of all time.",
      },
      {
        "Which band released the album 'Abbey Road'?": {
          "The Beatles": true,
          "The Rolling Stones": false,
          "Led Zeppelin": false,
          "Pink Floyd": false,
        },
        explanation:
          "The Beatles released 'Abbey Road' in 1969, one of their most famous albums.",
      },
      {
        "Which animated film features the song 'Let It Go'?": {
          Frozen: true,
          Moana: false,
          Tangled: false,
          "The Lion King": false,
        },
        explanation:
          "'Let It Go' is a popular song from the 2013 Disney film Frozen.",
      },
      {
        "Who played the Joker in 'The Dark Knight'?": {
          "Heath Ledger": true,
          "Joaquin Phoenix": false,
          "Jack Nicholson": false,
          "Jared Leto": false,
        },
        explanation:
          "Heath Ledger portrayed the Joker in the 2008 film 'The Dark Knight'.",
      },
      {
        "Which video game series features a character named Link?": {
          "The Legend of Zelda": true,
          "Super Mario": false,
          "Final Fantasy": false,
          Metroid: false,
        },
        explanation:
          "Link is the main character in the video game series 'The Legend of Zelda'.",
      },
      {
        "What is the name of the wizarding school in Harry Potter?": {
          Hogwarts: true,
          Beauxbatons: false,
          Durmstrang: false,
          Ilvermorny: false,
        },
        explanation:
          "Hogwarts is the wizarding school attended by Harry Potter and his friends.",
      },
      {
        "Who directed the movie 'Inception'?": {
          "Christopher Nolan": true,
          "Quentin Tarantino": false,
          "Martin Scorsese": false,
          "James Cameron": false,
        },
        explanation:
          "Christopher Nolan directed the movie 'Inception', which was released in 2010.",
      },
    ],
  ],
  culture: [
    [
      {
        "Which country is famous for its cherry blossoms?": {
          Japan: true,
          China: false,
          "South Korea": false,
          Thailand: false,
        },
        explanation:
          "Japan is renowned for its cherry blossoms, which bloom in spring.",
      },
      {
        "Which city is known as the birthplace of jazz music?": {
          "New Orleans": true,
          Chicago: false,
          "New York": false,
          "Los Angeles": false,
        },
        explanation:
          "New Orleans, Louisiana, is considered the birthplace of jazz music.",
      },
      {
        "What is the traditional garment worn in India called?": {
          Sari: true,
          Kimono: false,
          Hanbok: false,
          Caftan: false,
        },
        explanation:
          "The Sari is a traditional garment commonly worn by women in India.",
      },
      {
        "Which festival is known as the Festival of Lights in India?": {
          Diwali: true,
          Holi: false,
          Eid: false,
          Christmas: false,
        },
        explanation:
          "Diwali, the Festival of Lights, is one of the major festivals celebrated in India.",
      },
      {
        "Which language is primarily spoken in Brazil?": {
          Portuguese: true,
          Spanish: false,
          French: false,
          English: false,
        },
        explanation:
          "Portuguese is the official and most widely spoken language in Brazil.",
      },
      {
        "Which ancient civilization built the pyramids of Giza?": {
          Egyptian: true,
          Mayan: false,
          Roman: false,
          Greek: false,
        },
        explanation:
          "The ancient Egyptians built the pyramids of Giza over 4,000 years ago.",
      },
      {
        "What is the name of the sacred river in Hinduism?": {
          Ganges: true,
          Nile: false,
          Yangtze: false,
          Amazon: false,
        },
        explanation:
          "The Ganges River is considered sacred in Hinduism and holds great religious significance.",
      },
      {
        "Which Mexican holiday is known for honoring deceased loved ones?": {
          "Día de los Muertos": true,
          "Cinco de Mayo": false,
          Carnaval: false,
          "Las Posadas": false,
        },
        explanation:
          "Día de los Muertos, or Day of the Dead, is a Mexican holiday celebrating deceased loved ones.",
      },
      {
        "Which famous artwork is housed in the Louvre Museum in Paris?": {
          "Mona Lisa": true,
          "The Last Supper": false,
          "Starry Night": false,
          "The Persistence of Memory": false,
        },
        explanation:
          "The Mona Lisa, painted by Leonardo da Vinci, is one of the most famous artworks housed in the Louvre Museum.",
      },
      {
        "What is the traditional drink of Argentina, made from dried leaves?": {
          Mate: true,
          Tequila: false,
          Sake: false,
          Chai: false,
        },
        explanation:
          "Mate is a traditional Argentine drink made from the dried leaves of the yerba mate plant.",
      },
    ],
  ],
};

let category = getCategory(selectedCategory, globalCategories);
let questions = getDificulty(selectedDificulty, category[1]);
let initialOrder = orderQuestions(questions);
let order = initialOrder.map(function (o) {
  return o;
});

skipButton.addEventListener("click", skipQuestion);
explanationButton.addEventListener("click", showExplain);
quitExplainButton.addEventListener("click", function () {
  explainMenu.style.display = "none";
  explainMenu.classList.remove("show");
  quitBlur();
  nextExplainButton.style.pointerEvents = "auto";
  explanationButton.style.pointerEvents = "auto";
});
nextExplainButton.addEventListener("click", skipQuestion);

function getCategory(category, categories) {
  let entriesCategoreis = Object.entries(categories);
  return entriesCategoreis.find(function (entrieCategory) {
    console.log(entrieCategory[0]);
    return entrieCategory[0] == category;
  });
}

function getDificulty(dificulty, questions) {
  return questions[dificulty];
}

function orderQuestions(questions) {
  let random = 0;
  let order = [];
  while (order.length < questions.length) {
    random = Math.floor(Math.random() * questions.length);
    if (!order.includes(random)) {
      order.push(random);
    }
  }
  return order;
}

function getQuestion(order, questions) {
  return questions[order[0]];
}

function getAnswer(answers) {
  let answersEntries = Object.entries(answers);
  return answersEntries.find(function () {
    return answersEntries[1];
  });
}

function setQuestion() {
  skipButton.textContent = "Skip";
  progress.style.width =
    parseInt(100 - (order.length * 100) / questions.length) + "%";
  let question = getQuestion(order, questions);
  if (question) {
    currentExplanation = Object.values(question)[1];
    h2.textContent = Object.keys(question)[0];
    setAnswers(Object.values(question)[0]);
    order.shift();
  } else {
    h2.style.display = "none";
    skipButton.style.display = "none";
    explanationButton.style.display = "None";
    answers.forEach(function (answer) {
      answer.style.display = "none";
    });
  }
}

function setAnswers(question) {
  setStartQuestionTransition();
  orderAnswer = orderAnswers(Object.keys(question));
  correctAnswer = getAnswer(question)[0];
  answers.forEach(function (answer, index) {
    answer.textContent = Object.keys(question)[orderAnswer[index]];
    answer.addEventListener("click", function () {
      if (!userSelection.includes(correctAnswer) && answered) {
        answered = false;
        userSelection.push({
          [h2.textContent]: [answer.textContent, correctAnswer],
          explanation: currentExplanation,
        });
        setEndQuestionTransition(correctAnswer, answer.textContent);
        if (answer.textContent == correctAnswer) {
          correct++;
        }
      }
    });
  });
  stopTimer = false;
}

function setStartQuestionTransition() {
  answers.forEach(function (answer) {
    answer.style.pointerEvents = "auto";
  });
  answered = true;
  explanationButton.style.display = "none";
  explainMenu.classList.remove("show");
  quitBlur();
  timer.textContent = seconds;
  stopTimer = false;
  seconds = 15;
  timer.textContent = seconds;
  manageTimer();
}

function setEndQuestionTransition(correctAnswer, userAnswer) {
  skipButton.textContent = "Next question";
  explanationButton.style.display = "block";
  stopTimer = true;
  answers.forEach(function (answer) {
    if (answer.textContent == userAnswer) {
      answer.classList.add("incorrect");
      answer.classList.remove("correct");
    }
    if (answer.textContent == correctAnswer) {
      answer.classList.remove("incorrect");
      answer.classList.add("correct");
    }
    answer.style.pointerEvents = "none";
  });
}

function skipQuestion() {
  answers.forEach(function (answer) {
    answer.classList.remove("incorrect");
    answer.classList.remove("correct");
  });
  stopTimer = true;
  clearTimeout(id);
  if (skipButton.textContent == "Skip") {
    skips++;
  }
  seconds = 15;
  setQuestion();
}

function orderAnswers(ans) {
  let order = [];
  while (order.length < answers.length) {
    random = Math.floor(Math.random() * ans.length);
    if (!order.includes(random)) {
      order.push(random);
    }
  }
  return order;
}

function contTimer() {
  if (seconds > 0 && !stopTimer) {
    seconds--;
    manageTimer();
    timer.textContent = seconds;
  } else if (!stopTimer) {
    setEndQuestionTransition(correctAnswer, null);
  }
}

function manageTimer() {
  if (stopTimer) {
    return;
  }
  if (seconds >= 0 && !stopTimer) {
    id = setTimeout(contTimer, 1000);
  }
}

function showExplain() {
  explanationButton.style.pointerEvents = "none";
  nextExplainButton.style.pointerEvents = "none";
  explainMenu.style.display = "flex";
  setTimeout(function () {
    applyBlur();
    explainMenu.classList.add("show");
  }, 100);
  let explain = explainMenu.querySelector(".text-explain");
  explain.textContent = currentExplanation;
}

function setColorTheme() {
  if (selectedCategory === "history") {
    categoryTitle.textContent = "History";
    body.classList.add("history");
    contentBox.style.backgroundColor = "#006284";
  } else if (selectedCategory === "science") {
    categoryTitle.textContent = "Science";
    body.classList.add("science");
    contentBox.style.backgroundColor = "#2B6B1A";
  } else if (selectedCategory === "culture") {
    categoryTitle.textContent = "Culture";
    body.classList.add("culture");
    contentBox.style.backgroundColor = "#961315";
  } else if (selectedCategory === "geography") {
    categoryTitle.textContent = "Geography";
    body.classList.add("geography");
    contentBox.style.backgroundColor = "#0B1184";
  } else if (selectedCategory === "entretainment") {
    categoryTitle.textContent = "Entretainment";
    body.classList.add("entretainment");
    contentBox.style.backgroundColor = "#A20679";
  }
}

function setDifficulty() {
  if (selectedDificulty == 0) {
    levelDifficulty.textContent = "Easy level";
    levelDifficulty.style.color = "#01B66E";
  } else if (selectedDificulty == 1) {
    levelDifficulty.textContent = "Mid level";
    levelDifficulty.style.color = "#BA8B00";
  } else if (selectedDificulty == 2) {
    levelDifficulty.textContent = "Hard level";
    levelDifficulty.style.color = "#FD0105";
  }
}
function applyBlur() {
  main.classList.add("apply-blur");
}
function quitBlur() {
  main.classList.remove("apply-blur");
}
function sendResults() {
  localStorage.setItem("quizPoints", correct);
  localStorage.setItem("totalQuestions", initialOrder.length);
  localStorage.setItem("quizSkips", skips);
}
setQuestion();
